...
public class PowerSelector {
     ...
     for(l=0;l<numAgents;++l){
         ...
         if(price[l]>price_upper){
            ...
            // System.out.println("The  price ["+l+"]: "+price[l]+"It is too high. Exceeds the limit price: "+price_upper);
             ...
             System.out.println(
                     "The price [\"+ l +\"] = \"+ price [l] +\" is lower than the established average price--> price_upper: "+price_upper);
    ...
    public AgentFeatures[] DataOrganizer_batt_Ascending(int numBatts,AgentFeatures[] arrAgentFeatures){
     ...
     for(l=0;l<numBatts;++l){
         ...
         System.out.println("Battery SOC is analyzed"+aid[l]+":  soc["+l+"] ="+soc[l]+"\n");
         arrange++;
         //It's saved in arranger's array
         aid_a[e]=aid[l];             
         p_a[e]=p[l];
     ...
     System.out.println("SOC is ordered from HIGHEST to lowest state of charge\n");
     //Show the results
     for(h=0;h<numBatts;h++){
         
         System.out.println("agentsB2["+h+"]"+"AID :"+agentsB2[h].GetArrayAgent_AID());
         System.out.println("agentsB2["+h+"]"+"power :"+agentsB2[h].GetArrayAgent_p()+"W");
    ...
    public AgentFeatures[]DataOrganizer_batt_Descending(int numBatts,AgentFeatures[] arrAgentFeatures){
     ...
     System.out.println("The SOC is ordered from LESS to the highest state of charge\n");
     //Show the results
     for(h=0;h<numBatts;h++){
     ...
     return agentsB2;   
        ...
        public float BatteryDecision(float aCD_,String name_batt,float p_batt,float soc_batt,float powerBaterias){
        
        float powerBaterias_=0;
        
        if(aCD_==1){
            System.out.println(p_batt+"(kW) "+"consume "+name_batt+" via automatic loading phase.\n");
            System.out.println(soc_batt+"(%) "+"remains "+name_batt+".\n");
            //powerBaterias=powerBaterias+p_batt;
            powerBaterias_=powerBaterias;
        }
        if(aCD_==-1){
            System.out.println(p_batt+"(kW) "+"contributes\n "+name_batt+" via automatic discharge phase.\n");
            System.out.println(soc_batt+"(%) "+"remains "+name_batt+".\n");
